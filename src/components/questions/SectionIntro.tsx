 ...
 * 增强:艺术字体 + 多重装饰背景 + 书法竖排 + rough.js 手绘圆环 + 飘带动效。
 */
...
export function SectionIntro({ type, phaseNumber, onStart }: SectionIntroProps) {
  ...
  const numCN = ['壹', '贰', '叁', '肆', '伍', '陆', '柒', '捌', '玖'][phaseNumber - 1] || String(phaseNumber)
  return (
    <div className="section-intro" style={{ position: 'relative', overflow: 'visible' }}>
      ...
      {/* 印章角标 */}
      <SealStamp char={numCN} color="var(--mirror)" style={{ position: 'absolute', top: '20px', left: '20px', width: '44px', height: '44px', opacity: 0.6 }} />
      {/* 书法竖排,填补两侧空白 */}
      ...
      {/* 手绘圆环,围绕题型图标 */}
      <RoughCircle color="var(--mirror)" seed={phaseNumber * 13 + 5} style={{ position: 'absolute', top: '12px', left: '50%', transform: 'translateX(-50%)', width: '120px', height: '120px', opacity: 0.4, pointerEvents: 'none' }} />
      {/* 祥云装饰,底部 */}
      ...
      <img src={asset(`/images/methods/${type}.svg`)} className="section-icon" alt="" width={72} height={72} aria-hidden="true" style={{ position: 'relative', zIndex: 1 }} />
