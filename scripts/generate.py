...
''')
    ...
    # Emit EVERY base implementation (they are self-contained), then let
    # ENABLED_TOOL_NAMES decide what the agent may actually call at runtime.
    # Rationale: routes like chat-mode context injection do
    # `from ...l5_tools.base_tools import web_search` unconditionally - with
    # enable-only emission that import breaks in products where the tool is
    # disabled (ImportError -> silent context loss + test failures).
    custom_names = {ct.get("name") for ct in custom_tools}
    emitted_base_names = []
    for t, impl in base_impls.items():
        if t in custom_names:
            continue  # custom tool takes precedence
        tool_definitions.append(impl)
        emitted_base_names.append(t)

    for t in enabled_tools:
        if t not in custom_names and t not in base_impls:
            print(f"  [!] 警告: enabled 工具 '{t}' 无生成实现，已跳过（避免未定义符号）")

    # Enabled set = configured tools that actually exist (base or custom).
    enabled_names = [t for t in enabled_tools if t in custom_names or t in base_impls]

    # Base tools list
    base_tools_list = "\n    ".join("%s," % n for n in emitted_base_names) if emitted_base_names else "# No base tools"

    # Enabled tools list (names only - the runtime filter)
    enabled_list_str = "\n    ".join('"%s",' % n for n in enabled_names) if enabled_names else "# No tools enabled"
...
Configured tools: {', '.join(enabled_tools) if enabled_tools else 'none'}
Defined base tools: {', '.join(emitted_base_names) if emitted_base_names else 'none'}
Enabled at runtime: {', '.join(enabled_names) if enabled_names else 'none'}
"""
...
# All defined base tools (registry completeness - /api/tools capability view)
BASE_TOOLS = [
    {base_tools_list}
]

# Subset the agent is allowed to call (from agent.yaml tools.enabled)
ENABLED_TOOL_NAMES = [
    {enabled_list_str}
]
'''.strip()
...
from .l5_tools.base_tools import BASE_TOOLS, ENABLED_TOOL_NAMES
...
def create_app() -> FastAPI:
    ...
    async def startup():
        ...
        # L5: Register tools - only the ENABLED subset is exposed to the agent
        # (BASE_TOOLS defines everything so route imports stay valid).
        _enabled = set(ENABLED_TOOL_NAMES)
        for tool in BASE_TOOLS:
            if getattr(tool, "name", "") in _enabled:
    ...
    if kb_id:
        try:
            ...
            if chunks:
                parts.append(
                    "[知识库引用 ({})]\\n".format(kb_id)
                    + "\\n".join(f"- {c.text[:300]} (来源: {c.source}, 相似度 {round(c.score, 3)})" for c in chunks)
                )
            else:
...
async def chat(request: ChatRequest):
    ...
    async def event_stream():
        ...
        # Emit done exactly once (the runtime's own done AgentEvent is
        # intentionally not re-emitted inside the loop).
...
def copy_static_templates(output_dir: str, framework: Optional[str] = None):
    ...
    def _ignore_generated(directory, entries):
        """shutil.copytree ignore callback: skip files produced by generate_*()
        so the static copy never overwrites generated output. Paths are
        compared POSIX-normalized (the Windows backslash mismatch was the
        root cause of the v0.5.1 overwrite bug)."""
        rel_dir = Path(directory).resolve().relative_to(app_src.resolve()).as_posix()
        skipped = []
        for entry in entries:
            rel = entry if rel_dir == "." else f"{rel_dir}/{entry}"
            if rel in GENERATED and (Path(directory) / entry).is_file():
    ...
    for name in ["requirements.txt", "requirements-dev.txt", "Dockerfile"]:
        src = TEMPLATES_DIR / "backend" / name
        if src.exists():
            ensure_dir(output_dir)
            copy_template(str(src), os.path.join(output_dir, name))

    # Ship the full backend test suite with every generated product so the
    # completeness checklist ("模板 pytest 通过") is verifiable out of the box.
    tests_src = TEMPLATES_DIR / "backend" / "tests"
    if tests_src.exists():
        shutil.copytree(tests_src, Path(output_dir) / "tests", dirs_exist_ok=True,
                        ignore=shutil.ignore_patterns("__pycache__"))

    # Scripts
    scripts_src = TEMPLATES_DIR / "backend" / "scripts"
    if scripts_src.exists():
        shutil.copytree(scripts_src, Path(output_dir) / "scripts",
                        dirs_exist_ok=True,
                        ignore=shutil.ignore_patterns("__pycache__"))
    ...
    shutil.copytree(fe_src, fe_dst, dirs_exist_ok=True,
                    ignore=shutil.ignore_patterns(
                        "node_modules", "dist", ".vite", "__pycache__"))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="generate.py",
        description="Generate a complete runnable agent project from agent.yaml.",
        epilog="未指定 --framework 时（TTY 交互环境）进入交互式选择；"
              "非交互环境必须显式指定。框架全景见 docs/framework-selection.md",
    )
    parser.add_argument("config", help="path to agent.yaml")
    parser.add_argument("output_dir", help="directory to generate the project into")
    parser.add_argument("--framework", choices=FRAMEWORKS, default=None,
                        help="agent runtime framework (M0.21); default: agent.yaml "
                             "framework field, else interactive selection")
    parser.add_argument("--framework-list", action="store_true",
    ...
    # Interactive fallback requires a TTY - in non-interactive contexts
    # (CI, agent-driven runs) a bare input() would hang forever.
    if framework is None:
        framework = config.get("framework") or config.get("agent_framework", {}).get("name")
    if framework is None:
        if not sys.stdin.isatty():
            print("Error: no --framework given and stdin is not a TTY. "
                  f"Pass --framework with one of: {', '.join(FRAMEWORKS)}")
            sys.exit(1)
