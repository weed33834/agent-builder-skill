...
>   目标：拒绝空壳——每项功能可验证真实工作，而非只有按钮和路由。**已全量完成 15 份**：01 提示词 / 02 沙箱 / 03 上下文 / 04 工具 / 05 记忆 / 06 模型 / 07 编排工作流 / 08 语音 / 09 定时任务 / 10 技能插件 / 11 评估 / 12 监控告警 / 13 权限 IAM / 14 Agent 生命周期 / 15 对话体验全集（**14 交互域 800+ 细节点**：消息展示/输入/附件/搜索/会话/工作空间/任务/能力库/反馈/通知/导航/错误降级/安全/高级交互）。每个 M 板块下方挂载对应深度规格入口）
...
> - **验收测试清单**：docs/acceptance-test.md（15 模块 114 条可执行验收项，逐条=步骤+预期结果，全绿=完整性验收通过；15 对话体验域已扩至 20 条覆盖 14 交互域）
