...
>   目标：拒绝空壳——每项功能可验证真实工作，而非只有按钮和路由。**已全量完成 26 份**：01 提示词 / 02 沙箱 / 03 上下文 / 04 工具 / 05 记忆 / 06 模型 / 07 编排工作流 / 08 语音 / 09 定时任务 / 10 技能插件 / 11 评估 / 12 监控告警 / 13 权限 IAM / 14 Agent 生命周期 / 15 对话体验全集（**14 交互域 800+ 细节点**）/ 16 企业级·组织级通用能力（**13 子域 ≈130 项**）/ 17 教训与坑全集（**14 域 ≈144 项**）/ 18 可接入生态与现成服务大全（**9 类 ≈100 项**）/ 19 布局与设计规范（**7 域 ≈74 项**：信息架构/布局模式/导航/内容分类/设计系统/响应式/关键页面规格）/ 20 底层基础能力（**7 域 ≈66 项**：流式输出/结构化输出/文本算法/检索算法/并发韧性/多模态/前端底层）/ 21 文档与辅助体系（**7 域 ≈56 项**：错误码/报错指令/报错文档/使用文档/开发者文档/运维文档/文档工程）/ 22 数据治理与数据资产（资产目录/元数据/血缘/质量/分类分级）/ 23 成本计费与配额治理（成本看板/价格表/预算/配额/账单/比价器）/ 24 测试与质量保障（测试金字塔/用例库/运行器/回归趋势/混沌/质量门禁）/ 25 性能工程（TTFT/TPOT/吞吐/压测/缓存/资源池/队列）/ 26 用户研究与增长（指标大盘/埋点/留存/漏斗/A-B 实验/反馈中心）。每个 M 板块下方挂载对应深度规格入口）
...
> - **验收测试清单**：docs/acceptance-test.md（26 模块 290 条可执行验收项，逐条=步骤+预期结果，全绿=完整性验收通过；15 对话体验域 20 条覆盖 14 交互域，16 企业级 30 条覆盖 13 子域，17 教训域 20 条覆盖 14 域，18 生态 14 条覆盖 9 类，19 布局 14 条覆盖 7 域，20 底层 14 条覆盖 7 域，21 文档 14 条覆盖 7 域，22 数据治理 14 条覆盖 7 域，23 成本计费 14 条覆盖 7 域，24 测试质量 14 条覆盖 7 域，25 性能 14 条覆盖 7 域，26 增长 14 条覆盖 7 域）
