...
const MM_I18N_RESOURCES = {
  zh: {
    ...
    take: {
      ...
      alert_alloc_sum: '总和须 = {total}(当前 {sum})。可点击「自动配平」一键补齐。',
      alert_matrix_incomplete: '请为每条陈述都选择同意程度。',
      alert_auction_over: '总出价不能超过预算 {budget}(当前 {sum})。',
      // 草稿恢复
  ...
  en: {
    ...
    take: {
      ...
      alert_alloc_sum: 'Total must = {total} (currently {sum}). Tap "Auto-Balance" to fix instantly.',
      alert_matrix_incomplete: 'Please rate your agreement for every statement.',
      alert_auction_over: 'Total bids cannot exceed the budget of {budget} (currently {sum}).',
      // Draft resume
    ...
    compare: {
      ...
      self_label: 'You',
      other_label: 'Them',
      no_result: 'You have no assessment yet — look in the mirror first.',
  ...
  ja: {
    ...
    take: {
      ...
      alert_alloc_sum: '合計 = {total} が必要（現在 {sum}）。「自動配分」で一括調整可能。',
      alert_matrix_incomplete: 'すべての陳述に同意度を選択してください。',
      alert_auction_over: '入札合計は予算 {budget} を超えられません（現在 {sum}）。',
      // 草稿復元
